...
public class CategoriaController {
	...
	@GetMapping
	public ResponseEntity<Object> getAll() {//getAll é o nome que dei p/ meu método pegar tudo o que todos os objetos que tem no banco
		return ResponseEntity.ok(categoriaRepository.findAll()); //findAll método do JPA, trás todos os objetos do banco.
	}

	@GetMapping("/{id}") // capturo um objeto no banco pelo id
	public ResponseEntity<Categoria> getById(@PathVariable Long Id) {//@PathVariable é o parâmetro concreto  "te entrego a garrafa p/ vc trazer cheia de água"
		return (ResponseEntity<Categoria>) categoriaRepository.findById(Id).map(resposta -> ResponseEntity.ok(resposta))// findById - método JPA |
				.orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build());
	}

	@PostMapping // passo uma informação e busco outra complementar a primeira, ou apenas passe.
	//a diferença do post e do get(no cabeçalho), é que o post só passa o parâmetro no corpo da reposição
	public ResponseEntity<Object> post(@Valid @RequestBody Categoria categoria) {//@Valid uso para marcar o que será validado || @RequestBody: indica que o parâmetro estará no corpo do objeto
		if (categoriaRepository.existsById(categoria.getId())) //existsById método jpa p/ ver se existe através do Id
			return ResponseEntity.status(HttpStatus.CREATED).body(categoriaRepository.save(categoria)); // aqui eu estou dizendo que se existir o objeto é pr'eu salvar no id que existe no banco.
		
		throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Essa categoria não existe!", null);
		// se não existir, vou lançar uma exceção do tipo request com a mensagem em questão sendo exibida.	
	}
    ...
    public ResponseEntity<Categoria> put(@Valid @RequestBody Categoria categoria){
        return categoriaRepository.findById(categoria.getId())
            ...
            .orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build()); // caso não exista, o status é o not found
